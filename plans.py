...
def save_row(plan_id, startdate, plan_row_number, recipe_names, notes):
    ...
    for i in range(len(recipe_ids)):
        ...
        if not recipe_id or recipe_id == default_recipe_id:
    ...
    if not plan_row_id:
        sql = "INSERT INTO plan_rows (plan_id, plan_row_date, recipe_0, notes_0, recipe_1, notes_1, recipe_2, notes_2, notes) \
            VALUES (:plan_id, :plan_row_date, :recipe_0, :notes_0, :recipe_1, :notes_1, :recipe_2, :notes_2, :notes)"
    else:
        sql = "UPDATE plan_rows SET recipe_0=:recipe_0, notes_0=:notes_0, recipe_1=:recipe_1, notes_1=:notes_1, recipe_2=:recipe_2, \
            notes_2=:notes_2, notes=:notes WHERE plan_id=:plan_id AND plan_row_date=:plan_row_date"
    try:
        db.session.execute(sql, {"plan_id":plan_id, "plan_row_date":plan_row_date, "recipe_0":recipe_ids[0], "notes_0":recipe_notes[0], \
            "recipe_1":recipe_ids[1], "notes_1":recipe_notes[1], "recipe_2":recipe_ids[2], "notes_2":recipe_notes[2], "notes":notes})
        db.session.commit()
    except:
        db.session.rollback()
        return False
